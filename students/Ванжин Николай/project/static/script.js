...
Vue.component('search', {
	...
	new Vue({
			...
			methods: {
				...
				addItem(item) {
					request('basket', 'POST', JSON.stringify(item)).then((response) => {
						if (response.result === 1) {
							if (!this.basketGoods.find((goodsItem) => parseInt(goodsItem.id) === parseInt(item.id))) {
								this.basketGoods.push(item);
								this.$set(item, 'quantity', 1);
							} else {
								this.basketGoods.find((goodsItem) => parseInt(goodsItem.id) === parseInt(item.id)).quantity += 1;
							};
							console.log(this.basketGoods);
				...
				fetchAddItem(item) {
					fetch(`${API_ROOT}/basket`, {
						...
						if (response.result === 1) {
							if (!this.basketGoods.find((goodsItem) => goodsItem.id === parseInt(item.id))) {
								this.basketGoods.push({
									...item,
									quantity: 1
								});
							} else {
								this.basketGoods.find((goodsItem) => goodsItem.id === parseInt(item.id)).quantity += 1;
							};
							console.log(this.basketGoods);
				...
				reduceItem(id) {
					request(`basket/${id}`, 'PUT').then((response) => {
						if (response.result === 1) {

							let itemQuantity = this.basketGoods.find((goodsItem) => parseInt(goodsItem.id) === parseInt(id)).quantity -= 1;
							console.log(this.basketGoods);
							if (itemQuantity === 0) {
								this.removeItem(id)
							}
				...
				removeItem(id) { // удаляю товар из корзины
					request(`basket/${id}`, "DELETE").then((response) => {

						if (response.result === 1) {
							if (this.basketGoods.find((goodsItem) => +goodsItem.id === parseInt(id))) {
								let newGoods = this.basketGoods.filter((item) => +item.id !== parseInt(id));
				...
				filteredGoods() {
					...
					return this.filterGoods = this.goods.filter((goodsItem) => regexp.test(goodsItem.title));
